/*==========================================================
 * Alternative contraction mapping code that uses an alternative value of the logit smoother.
 * For detailed description of input and output arguments of this function, see contrMap_ES.c, which has exactly the same structure.
 * This file is only used for the referee response and not the material in the paper.
 * Written by Stefan Weiergraeber, May 2021
 *========================================================*/
#include "mex.h"
#include "math.h"
#include "stdio.h"
              ...
              double *pChurni, double *cSurplusPCWi, double *cSurplusNoPCWi, double *ccpMat)
{
    ...
    const double LogitSmoother = 0.25;        // factor to smooth binary choice probabilities.
    const double invLS = 1.0 / LogitSmoother; // inverse of logit smoother above.

    // Think about whether it makes sense to hard code various numbers here.
    // number of products, number of simulated consumers, etc.
    ...
    // Since model is myopic, we can solve contraction mapping market by market.
    for (mkt = 0; mkt < nM; mkt++)
    {
        ...
        // I believe this should be safe since none of the objects changes its size.
        ...
        // incValMx = mxCreateDoubleMatrix((mwSize)nJM * nI, 1, mxREAL);
        // incValM = mxGetDoubles(incValMx);
        ...
        // incValBeliefMx = mxCreateDoubleMatrix((mwSize)nJM * nI, 1, mxREAL);
        // incValBeliefM = mxGetDoubles(incValBeliefMx);
        ...
        // pSharesPCWMx = mxCreateDoubleMatrix(nJM, 1, mxREAL);
        // pSharesPCWM = mxGetDoubles(pSharesPCWMx);
        ...
        for (i = 0; i < nI; i++) // loop over consumers.
        {
            ...
            for (k_lag = 0; k_lag < nJM; k_lag++)
            {
                for (j = 0; j < nJM; j++)
                {
                    ...
                    // mexPrintf("awareiAdv[%d]: %f \n", mkt * nJM * nJM * nI + i * nJM * nJM + nJM * k_lag + j, aware_adv[mkt * nJM * nJM * nI + i * nJM * nJM + nJM * k_lag + j]);
                    // mexPrintf("awareiAdvM[%d]: %f \n", i * nJM * nJM + k_lag * nJM + j, awareiAdvM[i * nJM * nJM + k_lag * nJM + j]);
        ...
        while (norm > tol && iter < maxiter)
        {
            ...
            PCWUsageAvg[mkt] = 0.0;
            // mexPrintf("Starting value for PCWUsageAvg[%d]: %f \n", mkt, PCWUsageAvg[mkt]);
            maxabsdev = 0.0;
            ...
            for (ii = 0; ii < nI; ii++)
            {
                ...
                for (k_lag = 0; k_lag < nJM; k_lag++)
                {
                    ...
                    for (k = 0; k < nJM; k++)
                    {
                        ...
                        //     ((k == 6) && (k_lag == 5)) ||
                        //     ((k == 7) && (k_lag == 8)) ||
                        //     ((k == 8) && (k_lag == 7))) // condition if SC are firm-specific.
                        {
        ...
        { // loop over simulated consumers.
            ...
            { // loop over (current products).
                ...
                if (k == 0 || k == 2 || k == 5 || k == 7) // first contract of two-contract firms.
                {
                    ...
                    // if (mkt == 0)
                    // {
                    //     mexPrintf("pChurniM[%d,%d]: %f \n", ii, k, pChurniM[((ii * nJM) + k)]);
                    //     // mexPrintf("pSharesiM[%d]: %f \n", k, pSharesiM[((ii * nJM) + k)]);
                    //     // mexPrintf("pSharesiPCWM[%d]: %f \n", k, pSharesiPCWM[((ii * nJM) + k)]);
                    // }
                }
